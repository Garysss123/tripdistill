...
const sources = [...new Map([
  ...unitedKingdomCountrySources,
  ...unitedKingdomClusters.flatMap((cluster) => cluster.sources),
  ...unitedKingdomGuides.flatMap((guide) => guide.sources)
    ...
    try {
      let response = await fetch(source.url, {
        ...
        headers: {
          ...
          Accept: 'text/html,application/xhtml+xml,application/pdf;q=0.9,*/*;q=0.8', Range: 'bytes=0-4095'
        }
      });
      ...
      const restricted = response.status === 401 || response.status === 403 || response.status === 416 || response.status === 429;
...
if (problems.length) {
  console.error(`United Kingdom source check failed with ${problems.length} problem(s):`);
  for (const problem of problems) console.error(`- ${problem}`);
  process.exitCode = 1;
} else {
  console.log(`United Kingdom source check passed: ${sources.length} unique official references checked; ${warnings.length} restricted/slow endpoint warning(s), no broken response.`);
}
