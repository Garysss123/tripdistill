...
const jobs=locales.flatMap(([locale,prefix])=>usaRoutes.map(route=>({label:prefix+route,run:async()=>{
 ...
 for(const [slug,asset] of [['boston','boston'],['philadelphia','philadelphia'],['washington-dc','dc'],['new-england','new-england'],['chicago','chicago'],['seattle','seattle'],['portland-oregon','portland-oregon']])if(route.startsWith(`/usa/${slug}/`)&&(!html.includes(`data-editorial-revision="${asset}-20260911"`)||!html.includes(`/css/${asset}-editorial.css?v=20260911-1`)))throw Error(`Old ${slug} template served instead of the editorial rewrite`);
}})));
...
for(const [asset,selector] of [['boston','.bo-cover'],['philadelphia','.ph-cover'],['dc','.dc-city-cover'],['new-england','.ne-region-cover'],['chicago','.ch-city-cover'],['seattle','.se-city-cover'],['portland-oregon','.po-city-cover']])jobs.push({label:asset+' editorial stylesheet',run:async()=>{const css=await(await get(`/css/${asset}-editorial.css?v=20260911-1`)).text();if(!css.includes(selector)||!css.includes('max-width:520px'))throw Error('Editorial/responsive CSS missing');}});
