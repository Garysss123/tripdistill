...
check(unitedKingdomClusters.length === 20, `Expected 20 United Kingdom hubs, found ${unitedKingdomClusters.length}`);
check(unitedKingdomGuides.length === 60, `Expected 60 United Kingdom guides, found ${unitedKingdomGuides.length}`);
check(routes.length === 81 && new Set(routes).size === 81, 'Expected 81 unique English United Kingdom routes');
check(new Set(unitedKingdomClusters.map((item) => item.family)).size === 20, 'United Kingdom hub families must be unique');
check(new Set(unitedKingdomGuides.map((item) => item.instrument)).size === 60, 'United Kingdom planning instruments must be unique');
check(new Set(unitedKingdomGuides.map((item) => item.layout)).size === 60, 'United Kingdom child layout labels must be unique');
check(new Set(unitedKingdomGuides.map((item) => item.image.src)).size === 60, 'United Kingdom guide images must be unique');
...
]) check(new Set(unitedKingdomClusters.map((item) => item[field])).size === unitedKingdomClusters.length, `United Kingdom ${label} must not be cloned across hubs`);
...
]) check(new Set(unitedKingdomGuides.map((item) => item[field])).size === unitedKingdomGuides.length, `United Kingdom ${label} must not be cloned across child guides`);

check(new Set(unitedKingdomGuides.map((item) => JSON.stringify(item.choices))).size === 60, 'United Kingdom three-way choice sets must be unique per child guide');
check(new Set(unitedKingdomGuides.map((item) => JSON.stringify(item.route))).size === 60, 'United Kingdom four-stage routes must be unique per child guide');
check(new Set(unitedKingdomGuides.map((item) => JSON.stringify(item.watch))).size === 60, 'United Kingdom failure-point sets must be unique per child guide');
...
for (const route of routes) {
  ...
  const floor = route === '/united-kingdom/' ? 7600 : depth === 2 ? 6500 : 7000;
  check(pageText.length >= floor, `${route}: rendered editorial copy below United Kingdom floor (${pageText.length} < ${floor})`);
  check(html.includes('/css/united-kingdom.css?v=20260920-1'), `${route}: United Kingdom CSS missing`);
  ...
  check(route === '/united-kingdom/' ? html.includes('"@type":"WebPage"') : html.includes('"@type":"Article"'), `${route}: page-level JSON-LD type missing`);
  ...
  const hub = unitedKingdomClusters.find((cluster) => route === `/united-kingdom/${cluster.slug}/` || route.startsWith(`/united-kingdom/${cluster.slug}/`));
  const credited = route === '/united-kingdom/' ? unitedKingdomClusters.map((cluster) => cluster.guides[0]) : hub?.guides || [];
  ...
  if (route === '/united-kingdom/') {
    check(dom.filter((node) => classHas(node, 'uk-country-card')).length === 20, `${route}: expected 20 hub cards`);
    check(dom.filter((node) => classHas(node, 'uk-network-grid')).length === 1, `${route}: national operating model missing`);
  } else if (unitedKingdomGuides.some((guide) => guide.url === route)) {
    check(html.includes('/css/united-kingdom-field.css?v=20260920-1'), `${route}: United Kingdom field CSS missing`);
    check(dom.filter((node) => classHas(node, 'uk-purpose')).length === 1, `${route}: independent purpose panel missing`);
    check(dom.filter((node) => classHas(node, 'uk-choice-switchboard')).length === 1, `${route}: choice switchboard missing`);
    check((html.match(/<li><b>0[1-4]<\/b><small>/g) || []).length === 4, `${route}: four route stages missing`);
    check(dom.filter((node) => classHas(node, 'uk-breakpoints')).length === 1, `${route}: failure-point section missing`);
    check(dom.filter((node) => classHas(node, 'uk-live-desk')).length === 1, `${route}: near-claim official-source panel missing`);
    check(dom.filter((node) => classHas(node, 'uk-related')).length === 1, `${route}: related-guide section missing`);
    check(/data-uk-layout="[^"]+"/.test(html) && /data-uk-instrument="[^"]+"/.test(html) && /data-uk-variant="(?:[1-9]|1[0-2])"/.test(html), `${route}: child visual instrument markers missing`);
    for (const [url] of unitedKingdomGuides.find((guide) => guide.url === route).sources) check(html.includes(url), `${route}: route-specific official source missing from rendered page`);
  } else {
    check(dom.filter((node) => classHas(node, 'uk-guide-card')).length === 3, `${route}: expected three child cards`);
    check(/data-uk-family="[^"]+"/.test(html) && /data-uk-hub-variant="[1-8]"/.test(html), `${route}: hub family/variant markers missing`);
  }
...
const search = JSON.parse(read('data/search-index.json')).filter((item) => item.url.startsWith('/united-kingdom/'));
check(search.length === 81 && new Set(search.map((item) => item.url)).size === 81, 'United Kingdom search index must contain exactly 81 unique routes');
...
check(header.includes('<!-- UNITED_KINGDOM_HEADER_START -->') && header.includes('data-nav-key="united-kingdom"'), 'United Kingdom header integration missing');
check(sidebar.includes('<!-- EUROPE_NAV_START -->') && sidebar.includes('<!-- UNITED_KINGDOM_NAV_START -->') && sidebar.includes('data-sidebar-id="united-kingdom"'), 'Europe/United Kingdom sidebar integration missing');
check(sidebar.includes('data-sidebar-id="france"') && sidebar.includes('data-sidebar-id="switzerland"'), 'France or Switzerland navigation was not preserved');
check(footer.includes('<!-- UNITED_KINGDOM_FOOTER_START -->'), 'United Kingdom footer integration missing');
check(home.includes('<!-- UNITED_KINGDOM_HOME_START -->') && home.includes('<!-- UNITED_KINGDOM_HOME_CREDIT_START -->'), 'United Kingdom home card or credit missing');
...
if (failures.length) {
  ...
  console.error(`United Kingdom audit: ${failures.length} failure(s).`);
  process.exitCode = 1;
} else {
  console.log(`United Kingdom audit passed: 20 hubs, 60 focused guides, 81 English / 405 five-language routes, 60 credited images and 60 unique planning instruments${englishOnly ? ' (localized file checks deferred)' : ''}.`);
}
