...
The next California group contains 16 locally integrated English rewrites: San Francisco, Los Angeles, San Diego and Sierra Parks, each with three children. It is not yet released in five languages. Its English source checks passed 186 served checks and 32 desktop/mobile renders, followed by focused body review. The Mission and Kings Canyon photographs now have versioned filenames because `/assets/` uses immutable caching; the replacements are by Clyde Charles Brown (CC BY-SA 4.0) and Famartin (CC BY-SA 3.0), with original Commons records and editing disclosures in the generated pages. All localized trees were regenerated and references checked before removing the two unused old photographs; Git history retains the originals. Reviewed batches `38a` through `38d`, plus `38z` search coverage, provide 34,870 current keys per locale without missing or stale entries. Final post-polish visual and production acceptance is still pending.
...
- Country hubs: `/australia/`, `/vietnam/`, `/malaysia/`, `/china/`, `/japan/`, `/south-korea/` and `/thailand/`
